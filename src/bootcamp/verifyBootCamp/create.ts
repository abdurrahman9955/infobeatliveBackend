...
export default class ClassVerifyService {
    ...
    static async createVerifyClass(data: {
      ...
      why?:string,        
        ...
        const existingVerification = await prisma.verifyBootcamp.findFirst({
          where: {
            userId: data.userId,
        ...
        if (existingVerification) {
          ...
          return prisma.verifyBootcamp.update({
        ...
        return prisma.verifyBootcamp.create({
          data: {
            userId: data.userId,
            bootcampId: data.bootcampId,
            country:data.country,        
  ...
  static async updateVerifyClass(data: {
    userId:string, bootcampId:string, isVerified:boolean, verify:any, subject:string,
   ...
   return await prisma.bootcamp.update({
      where: { id: data.bootcampId, creatorId: data.userId },
       data: {
  ...
  static async getClassVerifyClassId(bootcampId:string) {
    return prisma.verifyBootcamp.findMany({ 
      where: { bootcampId },
      include: {
        user: {
  ...
  static async getVerifyClass(id: string) {
    return prisma.verifyBootcamp.findUnique({
     where: { id },
     include: {
      user: {
        include: {
  ...
  static async deleteVerifyClass(id: string) {
    return prisma.verifyBootcamp.delete({ where: { id } });
  }
}
