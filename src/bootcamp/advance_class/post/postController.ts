...
export const getPostsByClassId = async (req: Request, res: Response) => {
  try {
    ...
    const posts = await prisma.bootcampClassMediaUpload.findMany({
      where: { classId: classId, level:'ADVANCED', ...wherePosts },
      include: {
        user: {
          include: {
            profile: true,
...
export const getPostsByUserId = async (req: Request, res: Response) => {
  try {
    ...
    const resolution = req.query.resolution as string; // Optional query param
    const posts = await prisma.bootcampClassMediaUpload.findMany({
      where: { classId:classId, userId: userId, level:'ADVANCED', ...wherePosts},
      include: {
        user: {
          include: {
            profile: true,
...
export const getPostById = async (req: Request, res: Response) => {
  try {
    ...
    const post = await prisma.bootcampClassMediaUpload.findUnique({
      where: { classId:classId, level:'ADVANCED', id:postId },
      include: {
        user: {
          include: {
            profile: true,
...
export const updatePost = async (req: Request, res: Response) => {
  try {
    ...
    const post = await prisma.bootcampClassMediaUpload.update({
      where: {classId, level:'ADVANCED', id },
