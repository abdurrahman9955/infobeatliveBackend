  ...
  try {
    ...
    for (let file of files) {
      if (type === 'IMAGE') {
        ...
        const key = `images/${userId}/${bootcampId}/image/${uniqueFilename}/${Date.now()}-${file.originalname}`;
        ...
        for (const resolution of resolutions) {
          ...
          const videoKey = `videos/${userId}/${bootcampId}/video/${resolution}/${uniqueFilename}/${Date.now()}-${file.originalname}`;
    ...
    const post = await prisma.bootcampMediaUpload.create({
      data: {
        bootcampId,
        userId,
        title,
...
export const getPostsByGroupId = async (req: Request, res: Response) => {
  try {
    ...
    const posts = await prisma.bootcampMediaUpload.findMany({
      where: { bootcampId: bootcampId, ...wherePosts },
      include: {
        user: {
          include: {
            profile: true,
...
export const getPostsByUserId = async (req: Request, res: Response) => {
  try {
    ...
    const resolution = req.query.resolution as string; // Optional query param
    const posts = await prisma.bootcampMediaUpload.findMany({
      where: { bootcampId:bootcampId, userId: userId, ...wherePosts },
      include: {
        user: {
          include: {
            profile: true,
...
export const getPostById = async (req: Request, res: Response) => {
  try {
    ...
    const post = await prisma.bootcampMediaUpload.findUnique({
      where: { bootcampId:bootcampId, id:postId },
      include: {
        user: {
          include: {
            profile: true,
...
export const updatePost = async (req: Request, res: Response) => {
  try {
    ...
    const post = await prisma.bootcampMediaUpload.update({
      where: {bootcampId, id },
...
export const deletePost = async (req: Request, res: Response) => {
  try {
    ...
    await prisma.bootcampMediaUpload.delete({
      where: {bootcampId, id},
    });
