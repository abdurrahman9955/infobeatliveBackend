...
export const getPostsByClassId = async (req: Request, res: Response) => {
  try {
    ...
    const posts = await prisma.bootcampCourseVideo.findMany({
      where: {level:'INTERMEDIATE', sectionId: sectionId },
      include: {
        instructor: {
          include: {
            profile: true,
...
export const getPostsByUserId = async (req: Request, res: Response) => {
  try {
    ...
    const resolution = req.query.resolution as string; // Optional query param
    const posts = await prisma.bootcampCourseVideo.findMany({
      where: { sectionId:sectionId,level:'INTERMEDIATE', instructorId: userId },
      include: {
        instructor: {
          include: {
            profile: true,
...
export const getPostById = async (req: Request, res: Response) => {
  try {
    ...
    const post = await prisma.bootcampCourseVideo.findUnique({
      where: { sectionId:sectionId, level:'INTERMEDIATE', id:postId },
      include: {
        instructor: {
          include: {
            profile: true,
...
export const updatePost = async (req: Request, res: Response) => {
  try {
    ...
    const post = await prisma.bootcampCourseVideo.update({
      where: {sectionId, level:'INTERMEDIATE', id },
      data: { title },
    });
...
export const deletePost = async (req: Request, res: Response) => {
  try {
    ...
    const post = await prisma.bootcampCourseVideo.findUnique({
      where: { id, level:'INTERMEDIATE', sectionId },
    });
