...
export const getAllSections = async (req: Request, res: Response) => {
  try {
    ...
    const courses = await prisma.bootcampCourseSection.findMany({
      where: {  courseId, level:'INTERMEDIATE',  },
      include: {
        instructor: {
          include: {
            profile: true, 
...
export const getSectionById = async (req: Request, res: Response) => {
  ...
  try {
    const course = await prisma.bootcampCourseSection.findUnique({
      where: { id, level:'INTERMEDIATE',  courseId,},
      include: {
        instructor: {
          include: {
            profile: true, 
...
export const deleteSection = async (req: Request, res: Response) => {
  ...
  try {
    console.log(" Updating section with id in the backend:", id, "and courseId:", courseId);
     ...
     const section = await prisma.bootcampCourseSection.findUnique({ where: {level:'INTERMEDIATE', id } });
    ...
    await prisma.bootcampCourseSection.delete({ where: { id,level:'INTERMEDIATE', courseId } });
