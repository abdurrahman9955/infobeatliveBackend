...
export const createPost = async (req: Request, res: Response) => {
  try {
    ...
    for (let file of files) {
      if (type === 'IMAGE') {
        ...
        for (const resolution of resolutions) {
          ...
          const videoKey = `videos/${userId}/${classId}/video/${resolution}/${uniqueFilename}/${Date.now()}-${file.originalname}`;
    ...
    const post = await prisma.bootcampClassMediaUpload.create({
      data: {
        classId,
        userId,
        title,
...
export const getPostsByClassId = async (req: Request, res: Response) => {
  try {
    ...
    const classId = req.params.groupId as string;
    ...
    const posts = await prisma.bootcampClassMediaUpload.findMany({
      where: { classId: classId,level:'INTERMEDIATE', ...wherePosts },
      include: {
        user: {
          include: {
            profile: true,
...
export const getPostsByUserId = async (req: Request, res: Response) => {
  try {
    ...
    const resolution = req.query.resolution as string; // Optional query param
    const posts = await prisma.bootcampClassMediaUpload.findMany({
      where: { classId:classId, userId: userId,level:'INTERMEDIATE', ...wherePosts},
      include: {
        user: {
          include: {
            profile: true,
...
export const getPostById = async (req: Request, res: Response) => {
  try {
    ...
    const post = await prisma.bootcampClassMediaUpload.findUnique({
      where: { classId:classId,level:'INTERMEDIATE', id:postId },
      include: {
        user: {
          include: {
            profile: true,
...
export const updatePost = async (req: Request, res: Response) => {
  try {
    ...
    const post = await prisma.bootcampClassMediaUpload.update({
      where: {classId,level:'INTERMEDIATE', id },
...
export const deletePost = async (req: Request, res: Response) => {
  try {
    ...
    await prisma.bootcampClassMediaUpload.delete({
      where: {classId,level:'INTERMEDIATE', id},
    });
