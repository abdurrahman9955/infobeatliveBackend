...
export const getPostsByClassId = async (req: Request, res: Response) => {
  try {
    ...
    const posts = await prisma.bootcampClassMediaUpload.findMany({
      where: { classId: classId, level:'BEGINNER', ...wherePosts },
      include: {
        user: {
          include: {
            profile: true,
...
export const getPostsByUserId = async (req: Request, res: Response) => {
  try {
    ...
    const resolution = req.query.resolution as string; // Optional query param
    const posts = await prisma.bootcampClassMediaUpload.findMany({
      where: { classId:classId,level:'BEGINNER', userId: userId, ...wherePosts},
      include: {
        user: {
          include: {
            profile: true,
...
export const getPostById = async (req: Request, res: Response) => {
  try {
    ...
    const post = await prisma.bootcampClassMediaUpload.findUnique({
      where: { classId:classId,level:'BEGINNER', id:postId },
      include: {
        user: {
          include: {
            profile: true,
...
export const deletePost = async (req: Request, res: Response) => {
  try {
    ...
    await prisma.bootcampClassMediaUpload.delete({
      where: {classId, id, level:'BEGINNER',},
    });
