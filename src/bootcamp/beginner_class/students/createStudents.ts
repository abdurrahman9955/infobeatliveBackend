...
class ClassMemberService {
  ...
  async getMembersByBootCamp(userId: string) {
    try {
      const bootcampEnrollments = await prisma.bootcampStudent.findMany({
      ...
      // Extract unique bootcamps
      const uniqueBootcamps = new Map();
  
      for (const enrollment of bootcampEnrollments) {
        const bootcamp = enrollment.class?.bootcamp;
        if (bootcamp && !uniqueBootcamps.has(bootcamp.id)) {
          uniqueBootcamps.set(bootcamp.id, bootcamp);
        }
      }
  
      return Array.from(uniqueBootcamps.values()); // Return unique bootcamps
    } catch (error) {
      if (error instanceof Error) {
        throw new Error(`Failed to get members: ${error.message}`);
      } else {
