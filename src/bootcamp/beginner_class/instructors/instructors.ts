...
class ClassInstructorController {
  ...
  async existClass(req: Request, res: Response): Promise<void> {
    try {
      const { userId } = req.params; 
      const { classId } = req.params; 

      // Ensure you have proper authorization here if needed
      await ClassInstructorService.existClass(userId, classId);
      res.status(204).end();
    } catch (error) {
      const typedError = error as Error;
  ...
  async getInstructors(req: Request, res: Response): Promise<void> {
   ...
   async getAllInstructors(req: Request, res: Response): Promise<void> {
    try {
      ...
      const instructors = await ClassInstructorService.getAllInstructors();
      res.status(200).json(instructors);
    } catch (error) {
      const typedError = error as Error;
      res.status(500).json({ message: 'Failed to get Instructor', error: typedError.message });
