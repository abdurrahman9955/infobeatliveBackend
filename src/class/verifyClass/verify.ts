...
export default class ClassVerifyController {
  ...
  static async create(req: Request, res: Response) {
    try {
      ...
      const classVerify = await ClassVerifyService.createVerifyClass({
        userId,
        classId,
        country,        
        state,          
      ...
      res.status(201).json(classVerify);
    } catch (error) {
  ...
  static async getAllVerifyByClassId(req: Request, res: Response) {
    try {
      ...
      const classPricings = await ClassVerifyService.getClassVerifyClassId(classId);
      res.json(classPricings);
    } catch (error) {
      res.status(500).json({ error: "Failed to retrieve class pricings" });
    }
