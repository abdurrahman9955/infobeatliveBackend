...
const classWebhookRouter = express.Router();
...
classWebhookRouter.post('/webhook',  express.raw({ type: 'application/json' }), async (req, res) => {
  ...
  try {
    ...
    if (eventType === 'order_created' || eventType === 'subscription_payment_success') {
      ...
      if (customData.academy_id || customData.level) {
        console.warn('this webhook is not for class, it is for academy class');
        return res.status(400).send('this webhook is not for class, it is for academy class');
      }
      ...
      if (user && existingClass) {
        ...
        existingClass?.id as any, existingClass?.purpose as any, 
        );
     }
        ...
        await prisma.cLassEarning.update({
          ...
          data: { 
           ...
           if (!classEarning) {
            ...
            await prisma.classSubscribed.create({
              data: {
                classId:customData.class_id,
                userId:customData.user_id,
                type: customData.subscription_type,
...
export default classWebhookRouter;
