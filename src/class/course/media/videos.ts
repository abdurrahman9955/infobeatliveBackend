...
export const createPost = async (req: Request, res: Response) => {
  try {
    ...
    const post = await prisma.courseMedia.create({
      data: {
        sectionId,
        instructorId,
        title,
...
export const getPostsByClassId = async (req: Request, res: Response) => {
  try {
    ...
    const posts = await prisma.courseMedia.findMany({
...
export const getPostsByUserId = async (req: Request, res: Response) => {
  try {
    ...
    const resolution = req.query.resolution as string; // Optional query param
    const posts = await prisma.courseMedia.findMany({
      where: { sectionId:sectionId, instructorId: userId },
      include: {
        instructor: {
          include: {
            profile: true,
...
export const getPostById = async (req: Request, res: Response) => {
  try {
    ...
    const post = await prisma.courseMedia.findUnique({
      where: { sectionId:sectionId, id:postId },
      include: {
        instructor: {
          include: {
            profile: true,
...
export const updatePost = async (req: Request, res: Response) => {
  try {
    ...
    const post = await prisma.courseMedia.update({
      where: {sectionId, id },
      data: { title },
    });
