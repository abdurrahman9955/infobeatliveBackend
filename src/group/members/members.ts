...
class GroupMemberController {
  ...
  async requestMember(req: Request, res: Response): Promise<void> {
    try {
      const { userId, groupId } = req.params;
      // Ensure proper authorization if needed
      const member = await GroupMemberService.requestMember(userId, groupId);
      res.status(201).json(member);
    } catch (error) {
      if (error instanceof Error) {
        res.status(500).json({ message: 'Failed to add member', error: error.message });
  ...
  async getMembers(req: Request, res: Response): Promise<void> {
    ...
    // Get all members of a group
    async getMembersByGroups(req: Request, res: Response): Promise<void> {
      try {
        const { userId } = req.params;
        const members = await GroupMemberService.getMembersByGroups(userId);
        res.status(200).json(members);
      } catch (error) {
        if (error instanceof Error) {
          res.status(500).json({ message: 'Failed to get members', error: error.message });
  ...
  // Get all members of a group
  async getRequestMembers(req: Request, res: Response): Promise<void> {
    try {
      const { groupId } = req.params;
      const members = await GroupMemberService.getRequestMembers(groupId);
      res.status(200).json(members);
    } catch (error) {
      if (error instanceof Error) {
        res.status(500).json({ message: 'Failed to get members', error: error.message });
